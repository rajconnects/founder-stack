...
Verify (layer 4) gets the sharpest tooling — that's where spec-to-code drift and unsafe migrations do the most damage. Layers 1.5, 2.5, and 6 are optional context-and-coherence layers added Apr 30, 2026 (morning), adapted from `mattpocock/skills` (`grill-with-docs`, `zoom-out`, `improve-codebase-architecture`). They share a portable contract: each reads `context_doc`, `glossary_doc`, and `decision_records` from `project.json`.

`/publish-gate` (layer 5, sibling of `/deploy-gate`) was added Apr 30, 2026 (evening) after the alignmink-dtp v0.3.0–0.3.5 incident series — four user-facing bugs in five releases, all of which would have been caught by installing the published tarball as a fresh consumer before declaring success. The gate runs `npm pack` → install in tmpdir → run a smoke command → assert expected output. Same release also added `real_corpora` (test-gate input) and `schemas_of_record` (handoff drift check) to `project.json`. See "Reality-check inputs" below.
...
| `hooks/*.sh` | Shell scripts wired via `settings.json`. `auto-lint.sh` + `pre-git-check.sh` are existing; `tsc-check.sh`, `migration-guard.sh`, `main-push-guard.sh` are new. |
...
A phase's quality ceiling is its brief's quality. Author briefs per `Build-Plans/component-briefs/AUTHORING.md` — phase label cross-check, Figma frame authority, utility-class resolution are caught there, once. Don't build downstream checks for what the brief should catch itself.
...
5. After a component/screen set is complete: `/design-gate <scope>` — `design-auditor` checks tokens, component contract, a11y, Figma alignment.
...
| Existing skill | Role in the new workflow |
|---|---|
| `dev-cycle` | Orchestrator of layer 3→4 pre-ship (lint + test + type-check + security + code review). Called before `/deploy-gate`. |
| `lint`, `test`, `code-review`, `security-review` | Components of `dev-cycle`. Keep. |
| `decision-trace-capture` | Invoked by `/handoff` for any decisions surfaced during the phase. |
| `auto-lint.sh` hook | Layer 3 rail. Unchanged. |
| `pre-git-check.sh` hook | Layer 3 rail — pre-commit/push lint + test gate. Unchanged. |
| `tsc-check.sh` hook (new) | Layer 3 — warns on TypeScript errors after Edit. Non-blocking. |
| `migration-guard.sh` hook (new) | Layer 4 — soft-warns if editing a migration without `/schema-gate` marker. |
| `main-push-guard.sh` hook (new) | Layer 5 — soft-warns on `git push main` without `/deploy-gate` marker. |
...
These are cleared at session end (add to gitignore; no cleanup needed between sessions since markers are session-scoped). They're advisory — not a hard lock. A missing marker triggers a warning, not a block.
...
4. Gitignore `.claude/settings.local.json`, `.claude/.schema-gate-passed`, `.claude/.deploy-gate-passed-*`, `.claude/.design-gate-passed`.
5. Confirm `Read` / `Grep` / `Bash` permissions for the new repo's paths.

If `stack.db` is not Supabase, `schema-analyst` will error out — either adapt `project.json` or extend that agent with alternative DB tools.

If `design_system.figma.file_key` is null, the Figma audit in `design-auditor` is skipped automatically.

## Gate assignment — which gate enforces which criterion

Acceptance criteria that require a running app, network, or DOM do NOT belong in `/design-gate` (which only reads files and the Figma MCP). Assign as follows:
...
| Figma alignment (variable match, visual) | `/design-gate` | Figma MCP available; static comparison |
...
Delegation has overhead (prompt, context handoff, review). For bounded work the overhead dominates; main agent does the work directly. The `test-gate` command classifies and branches automatically — see `.claude/commands/test-gate.md`.

**Still mandatory for bounded work:** `/design-gate` (static token/Figma audit) and, for UI work, `/deploy-gate` runtime checks. Gates are the whole point; delegation within a gate is the cost-control lever.
...
Some projects keep static mockup files (e.g., `apps/ui-mockups/consolidated-mockup.html`) as visual references during design sprints. These are NOT authoritative — the `design-auditor` compares only against `design_system.components_spec`, `flow_spec`, `tokens`, `copy_guide`, and `figma` from `project.json`. Mockups are historical artifacts; drift between mockups and spec is expected.
...
| `/schema-gate` skips live-state checks | Supabase MCP not authenticated | Surface warning, don't block; authenticate and re-run |
...
## The three context-and-coherence commands (added Apr 30, 2026)
...
- `json-traces` — Alignmink-style decision-trace JSON (with `topic`, `status`, `resolution_summary`, `revisit_trigger`).
...
## Reality-check inputs (added Apr 30, 2026, evening)

The `mattpocock/skills` additions in the morning gave the workflow a **before-acting** layer (do you understand the context?). The evening additions give it an **after-acting** layer (does the artifact actually work?). Both share the same pattern: declare what matters in `project.json`, gates parameterize on it.
...
A gate has a single job (one question, one verdict). A config field tells an existing gate where to look. `release_artifacts` is "where my publishable thing is + how to smoke it." `real_corpora` is "where my real-world fixtures live + how to validate them." Adding three new gates would inflate the workflow surface area; teaching existing gates new tricks via config does not.

### The bug class each one targets

- **`release_artifacts` example** — alignmink-dtp v0.3.0 shipped with a `postinstall` hook pointing at `scripts/postinstall-hint.mjs`, but `scripts/` was missing from the `files` allowlist. Every consumer install crashed. `prepublishOnly` ran unit tests against the source tree, which had the file. Only a fresh-consumer install would have caught this.
- **`real_corpora` example** — alignmink-dtp v0.3.3 made `decision.reasoning` required in the Zod schema. The fixture had it. The 12 real traces on disk did not. Tests passed; production rejected the actual data.
- **`schemas_of_record` example** — alignmink-dtp v0.3.4 fixed a bug where the Cowork producer template emitted shorthand-shape nodes that the reader rejected. Root cause: the template diverged from the canonical JSON Schema over multiple releases because nothing checked them against each other.
...
4. **Required additionally for layers 1.5, 2.5, and 6 (the morning commands):**
...
5. **Required additionally for the reality-check inputs (the evening additions):**
...
If `stack.db` is not Supabase, `/schema-gate` errors out (unchanged behavior).

If `design_system.figma.file_key` is null, the Figma audit is skipped (unchanged behavior).
